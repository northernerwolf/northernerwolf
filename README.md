
![header](https://capsule-render.vercel.app/api?type=waving&color=gradient&height=200&section=header&text=Welcome&fontSize=72&animation=fadeIn&fontAlignY=26&desc=&descAlignY=50&descAlign=50&fontAlign=77)






### Fullstack Developer

**Android · iOS · Flutter · React Native · Node.js · TypeScript**


---

## About me

Mobile App & Fullstack Developer with **5+ years of commercial experience** building complete, production-ready products — from mobile client to backend API and database. I specialise in shipping polished Android and iOS apps (Kotlin, Swift, Flutter, React Native) backed by robust server-side systems (Node.js, TypeScript, PostgreSQL). Experienced in remote teams, CI/CD, Firebase ecosystem, and app publishing on Google Play and App Store.

---

## Tech stack

###  Mobile
`Kotlin` `Java` `Swift` `Flutter` `React Native` `Jetpack Compose` `Android SDK` `Compose Multiplatform`

###  Backend & API
`Node.js` `TypeScript` `Express` `REST APIs` `GraphQL` `WebSockets`

###  Data & Cloud
`PostgreSQL` `MongoDB` `Firebase` `RevenueCat` `FCM`

###  Architecture & DevOps
`Clean Architecture` `MVVM` `Dependency Injection` `Docker` `GitLab CI/CD` `Git` `Scrum`

---

## Featured projects

### 🎵 [Aýdymçy — Music Streaming](https://play.google.com/store/apps/details?id=com.googadev.lyrics_app&hl=tr)
Full music streaming platform with lyrics support, optimised media playback, and backend API.
`Kotlin` `Node.js` `ExoPlayer` `REST` `Google Play`

### 🚗 [Kerwen Taxi](https://play.google.com/store/apps/details?id=com.kerwen.taxi) + [Kerwen Driver](https://play.google.com/store/apps/details?id=com.kerwen.driver)
Full ride-hailing suite — passenger booking app and driver companion with real-time tracking and backend services.
`Kotlin` `Node.js` `Google Maps` `WebSockets` `FCM` `Background Services`

### 📦 [106 Cargo — Logistics](https://play.google.com/store/apps/details?id=guwanchaldurdyyewProduct.kargo_app&hl=en)
Cargo tracking app with real-time data integration and reliable backend API.
`Flutter` `REST` `PostgreSQL`

### 📖 [ElKitap — E-Reader](https://play.google.com/store/apps/details?id=com.googadev.elkitap)
Digital reading app with clean UI and smooth reading experience.
`Flutter` `Google Play` `App Store`

### 🍽️ Restaurant Menu Suite
5 digital menu apps (Kebapchy, Balykchy, Mado, Gala, DaVinci) published on Google Play & App Store.
`Flutter` `Android` `iOS`

---


<div align="center" >

[![My Skills](https://skillicons.dev/icons?i=androidstudio,java,kotlin,swift,nodejs,react,cpp,flutter,dart,firebase,gradle,xd,ai,figma&theme=light)](https://skillicons.dev)

<a href="https://instagram.com/guwanc_haldurdyyev" target="_blank"><img src="https://img.shields.io/badge/-Instagram-%23E4405F?style=for-the-badge&logo=instagram&logoColor=white" target="_blank">
 </a><a href="https://www.linkedin.com/in/guwanch-haldurdyyev-7b8a2a243" target="_blank">
  <img src="https://img.shields.io/static/v1?style=for-the-badge&message=LinkedIn&color=0A66C2&logo=LinkedIn&logoColor=FFFFFF&label" target="_blank">
 </a> <a href="https://www.facebook.com/profile.php?id=100093098223124&mibextid=ZbWKwL" target="_blank">
  <img src="https://img.shields.io/static/v1?style=for-the-badge&message=Facebook&color=1877F2&logo=Facebook&logoColor=FFFFFF&label=" target="_blank">
 </a><a href="https://t.me/northern_wolf00" target="_blank">
     <img src="https://img.shields.io/static/v1?style=for-the-badge&message=Telegram&color=26A5E4&logo=Telegram&logoColor=FFFFFF&label=" target="_blank">
 </a><a href = "https://x.com/northernerwolf7?t=yv25QRaObsG-nQKI0stpVw&s=35"  target="_blank"><img src="https://img.shields.io/static/v1?style=for-the-badge&message=Twitter&color=1DA1F2&logo=Twitter&logoColor=FFFFFF&label=" target="_blank"></a>
  <a href ="https://discordapp.com/users/wolf290300"  target="_blank"><img src="https://img.shields.io/static/v1?style=for-the-badge&message=Discord&color=5865F2&logo=Discord&logoColor=FFFFFF&label=" target="_blank"></a>
 <a href ="mailto:guwanchaldurdyyew01@gmail.com"  target="_blank"><img src="https://img.shields.io/badge/-Gmail-%23333?style=for-the-badge&logo=gmail&logoColor=white" target="_blank"></a>

 📍 Ashgabat, Turkmenistan · 📧 guwanchaldurdyyew01@gmail.com · [🌐 Portfolio](https://northernwolf-portfolio.vercel.app)

![Open to remote](https://img.shields.io/badge/Open%20to%20remote%20opportunities-worldwide-1D9E75?style=flat)
  
</div>

 <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=100&section=footer"/>
























